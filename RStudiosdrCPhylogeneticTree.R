...
#dropping tips for which don't have good protein sequences (all sequences w/ extra 90 amino acids + reference)
rootedMAviumPatientTree = ape::drop.tip(rootedMAviumPatientTree, "SRR14736599")
rootedMAviumPatientTree = ape::drop.tip(rootedMAviumPatientTree, "SRR14736598")
rootedMAviumPatientTree = ape::drop.tip(rootedMAviumPatientTree, "SRR14736592")
rootedMAviumPatientTree = ape::drop.tip(rootedMAviumPatientTree, "Reference")
...
#multiple sequence alignment <-- hard to interpret
#give multi fasta version of your alignment
#AA_sequence = "/Users/charanjeetdhillon/Documents/MAvium/ClustalWproteinAlignmentSdrCWOXtraAminoAcids.fasta"
#TreePlotwSeqAlign <- Treeplot #+ geom_tiplab(offset=4, align=TRUE) + 
#msaplot(TreePlotwSeqAlign, AA_sequence, window=c(1, 332))

#creating a heat map on the sites of variation only in the sdrC gene sequences for all strains + reference
#creating a heat map/tile plot on the sites of variation only in the sdrC gene sequences for all strains + reference
...
clustalWSeqFileName <- "/Users/charanjeetdhillon/Documents/MAvium/Multifastas/sdrCSeqforWOAdditionalAminoAcids.fasta"
#read in the aligned sequence file w/o weird gene sequences
alignedSeqs <- seqinr::read.fasta(file = clustalWSeqFileName)
